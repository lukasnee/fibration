...
static constexpr std::size_t sampleBufferSize =
    0x20; // TODO experiment on this one
...
void power2(BufferF32<size> &in, BufferF32<size> &out) {
    arm_mult_f32(in.data(), in.data(), out.data(), out.size());
}

template <std::size_t size>
void power4(BufferF32<size> &in, BufferF32<size> &out) {
    power2(in, out);
    power2(out, out);
}
// TODO: SampleTime as class with arithmetic operands overrides (parallel
