...
void convert(const I2sSampleBufferU32 &from, StereoSampleBufferQ31 &to) {
    constexpr std::size_t frameToSampleBitDiff =
        std::numeric_limits<U32>::digits - bitDepth;
    for (std::size_t i = 0; i < to.left.size(); i++) {
        to.left[i] = rawSampleToQ31<bitDepth>(swap(from[i].left)
                                              << frameToSampleBitDiff);
        to.right[i] = rawSampleToQ31<bitDepth>(swap(from[i].right)
                                               << frameToSampleBitDiff);
    }
}

template <std::size_t bitDepth>
void convert(const StereoSampleBufferQ31 &from, I2sSampleBufferU32 &to) {
    constexpr std::size_t frameToSampleBitDiff =
        std::numeric_limits<U32>::digits - bitDepth;
    for (std::size_t i = 0; i < from.left.size(); i++) {
        to[i].left = swap(q31ToRawSample<bitDepth>(from.left[i]) >>
                          frameToSampleBitDiff);
        to[i].right = swap(q31ToRawSample<bitDepth>(from.right[i]) >>
                           frameToSampleBitDiff);
    }
}
...
template <std::size_t bitDepth>
void convert(StereoSampleBufferF32 &from, I2sSampleBufferU32 &to) {
    StereoSampleBufferQ31 tmp;
