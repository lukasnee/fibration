        ...
        [](Shell &shell, std::size_t argc, const char *argv[]) -> Shell::Command::Result
        {
            ...
            for (;;)
            {
                ...
                for (j = 0; 2 * PI > j; j += 0.07 / scale)
                    ...
                    {
                        float c = arm_sin_f32(i), d = arm_cos_f32(j), e = arm_sin_f32(A), f = arm_sin_f32(j), g = arm_cos_f32(A), h = d + 2, D = 1 / (c * h * e + f * g + 5), l = arm_cos_f32(i), m = arm_cos_f32(B), n = arm_sin_f32(B), t = c * h * g - f * e;
                        int x = canvasWidth / 2 + 30 * D * (l * h * m - t * n), y = 12 + 15 * D * (l * h * n + t * m), o = x + canvasWidth * y, N = 8 * ((f * e - c * d * g) * m - c * d * e - f * g - l * d * n);
