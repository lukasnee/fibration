...
I2sStream i2s2Stream(Periph::getI2s2(), "i2s2stream", 4 * 1024 / sizeof(StackType_t), FibSys::Priority::audioStream,
                     i2s2StreamBuffer,
                     []([[maybe_unused]] const Fib::Dsp::StereoSampleBufferF32 &rxStereoSampleBlock,
                        [[maybe_unused]] Fib::Dsp::StereoSampleBufferF32 &txStereoSampleBuffer) {
                         ...
                         // pWaves[0]->amplitudeNormal.set(pWaves[0]->frequencyInHz.get() < rolloff ?
                         // pWaves[0]->frequencyInHz.get() / rolloff : 1.f);
                         // pWaves[1]->amplitudeNormal.set(pWaves[1]->frequencyInHz.get() < rolloff ?
                         // pWaves[1]->frequencyInHz.get() / rolloff : 1.f);
                         // pWaves[2]->amplitudeNormal.set(pWaves[2]->frequencyInHz.get() < rolloff ?
                         // pWaves[2]->frequencyInHz.get() / rolloff : 1.f);
                         // pWaves[3]->amplitudeNormal.set(pWaves[3]->frequencyInHz.get() < rolloff ?
                         ...
                         static float coeff[] = {
                             0.0000000f,  -0.0004873f, -0.0019827f, -0.0044985f, -0.0079935f, -0.0123743f, -0.0175003f,
                         ...
                         // float biquada[] = {0.9603640455554292, -1.9576260295124621, 0.9069191056867634,
                         // -1.904255735525569}; float biquadb[] = {1, -2, 1, -2};

                         static bool filterInitialized = false;
                         if (!filterInitialized) {
                             filterInitialized = true;
                             // arm_iir_lattice_init_f32(&iirInstace, (sizeof(biquada)/sizeof(float)), biquada, biquadb,
...

                         {
                             ...
                             static Fib::PeriodicTimerApp higgsStats("hs", 10.f, [&]() {
                                 LOG_DEBUG("%8.2f %8.2f %8.2f %8.2f %8.2f %8.2f %8.2f %8.2f",
